...
namespace FormGenerator.API.Controllers.Shared
{
    ...
    {
        //private string apiUrl = "http://resturant-api.bird-cloud.com/";
        ...
        [HttpPost]
        public async Task<ResultViewModel> Upload()
        {
        ...
        [HttpGet]
        public ResultViewModel DeleteAttachment(string fileName)
        {
            ResultViewModel resultViewModel = new ResultViewModel();
            try
            {
                ...
                if (File.Exists(path))
                    File.Delete(path);
                resultViewModel.Message = Resource.SuccessfullyDeleted;
            }
            catch (Exception ex)
            {
                resultViewModel.Success = false;
